...
// обновление количества найденных по фильтру объъектов
...
if (number > 0) {
    ...
    // напрямую получаем доступ к объекту и записываем в него значение
...
export function getInput(){
    ...
    // получаем значение из select - ЖК
    if (elements.filterSelect[0].value !== 'all'){
        searchParams.append(elements.filterSelect[0].name, elements.filterSelect[0].value) // добавляем параметры в формируемую строку запроса
    }

    // получаем значение из чекбокса - количество комнат
    const roomsValues = [];
    ...
    // получаем значение из текстовых полей input - площади и цены 
