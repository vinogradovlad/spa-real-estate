export function render(object, isFaved) {
    console.log(isFaved); // ture or false при нажатии Добавить в избранное
...
// будет создавать объект с данными json для отпарвки на сервер
export function getInput() {
    // создаю новый объект
    const formData = {};
    // создаю в нем свойства name и phone, куда буду записывать данные модального окна из форм с именем и телефоном
    formData.name = document.querySelector('#form-name').value;
    formData.phone = document.querySelector('#form-phone').value;
    // данная функция будет возвращать созданный объект
    return formData;
}
